...
t.test('must be called with new', t => {
  t.equal(typeof Stacker, 'function');
...
t.test('ok if called without a process object', t => {
  ...
  const stacker = new Stacker({ internals: [] })
  t.equal(stacker.clean(raw), expected, 'no cwd is stripped off')
  t.end()
})
...
t.test('clean: truncates cwd', t => {
  ...
  let stacker = new Stacker({cwd: '/user/dev/project', internals: []});
  t.equal(stacker.clean(LinuxStack1), expected, 'accepts a linux string');
  t.equal(stacker.clean(LinuxStack1.split('\n')), expected, 'accepts an array');
  t.equal(stacker.clean(LinuxStack1.split('\n').slice(1)), expected, 'slices off the message line');

  stacker = new Stacker({cwd: 'Z:\\user\\dev\\project', internals: []});
  t.equal(stacker.clean(WindowsStack1), expected, 'accepts a windows string');

  const expectIndented = utils.join(expectedArray.map(a => '    ' + a));
  t.equal(stacker.clean(WindowsStack1, 4), expectIndented, 'indentation');

  t.end();
});

t.test('clean: eliminates internals', t => {
  let stacker = new Stacker({cwd: '/user/dev'});
  ...
  t.equal(stacker.clean(LinuxStack1), expected);

  stacker = new Stacker({cwd: 'Z:\\user\\dev'});
  t.equal(stacker.clean(WindowsStack1), expected);
  t.end();
});

t.test('clean: returns null if it is all internals', t => {
  const stacker = new Stacker();
  t.equal(stacker.clean(utils.join(internalStack())), '');
  t.end();
});

t.test('captureString: two redirects', t => {
  const stacker = new Stacker({internals: internals(), cwd: utils.fixtureDir});
  const capture = new CaptureFixture(stacker);
...
t.test('capture: with wrapCallSite function', t => {
  ...
  const stacker = new Stacker({internals: internals(), cwd: utils.fixtureDir, wrapCallSite: wrapper});
  const capture = new CaptureFixture(stacker);
  const stack = capture.redirect1('redirect2', 'call', 'capture', 1, capture.call);
  t.equal(stack.length, 1);
...
t.test('at: with stackStart', t => {
  const stacker = new Stacker({internals: internals(), cwd: __dirname});
  const capture = new CaptureFixture(stacker);
...
t.test('parseLine', t => {
  const stack = new Stacker({internals: internals(), cwd: '/user/dev/project'});
...
t.test('parseLine: handles native errors', t => {
  ...
  t.same(stack.parseLine('    at Error (native)'), {
    native: true,
    function: 'Error'
  });
  t.end();
...
function internals() {
  return Stacker.nodeInternals().concat([
    /test\.js:\d+:\d+\)?$/,
    /\/node_modules\//
  ]);
}
